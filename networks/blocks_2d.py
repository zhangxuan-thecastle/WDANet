...
class DePE(nn.Module):  #输入(B, C, H, W)  输出(B, C, H, W) 内容经过多层卷积处理和叠加，可能具有更丰富的特征表示
    def __init__(self, dim_in, dim_out, k_sizes=[3, 5], conv_op=nn.Conv2d, groups=None):
        #dim_in: 输入特征图的通道数  dim_out: 输出特征图的通道数  k_sizes: 卷积核大小的列表，默认为 [3, 5]。  c
        super().__init__()
        if groups is None:
            self.groups = dim_out
        else:
        ...
        #onv_op: 卷积操作，默认为 nn.Conv2d。 groups: 卷积的分组数，默认为 dim_out。
    ...
    def forward(self, x):
        ...
        if self.use_pe and (not self.no_off):
            if self.dwc_pe:
                ...
                # print(rpe_table.shape)
                attn_bias = rpe_table[None, ...].expand(B, -1, -1, -1)
                # print(self.q_h, self.kv_w, self.stride, rpe_table.shape, attn_bias.shape, attn.shape)
                ...
                # print(attn_bias.shape, attn.shape)
